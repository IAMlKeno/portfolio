...
/**
 * This function will will access the database and retrieve projects related to the tag passed.
 *
 * @param $db
 * @param $tag
 * @return array
        ...
        while($row = $result->fetch_assoc()){
            ...
            if($resultTwo->num_rows > 0){
                while($row = $resultTwo->fetch_assoc()){
                    $tempProject = new Project($row["projectId"], $row["projectTitle"], $row["description"],
                        $row["link"], $row["linkDescription"], $row["imageLocation"], $row["projectType"], $row["tags"]);
